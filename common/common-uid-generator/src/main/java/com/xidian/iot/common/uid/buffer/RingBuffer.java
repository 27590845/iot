 ...
 */
package com.xidian.iot.common.uid.buffer;

import java.util.concurrent.atomic.AtomicLong;

import com.xidian.iot.common.uid.utils.PaddedAtomicLong;
...
public class RingBuffer {
    ...
    public RingBuffer(int bufferSize, int paddingFactor) {
        ...
        isTrue(bufferSize > 0L, "RingBuffer size must be positive");
        isTrue(Integer.bitCount(bufferSize) == 1, "RingBuffer size must be a power of 2");
        isTrue(paddingFactor > 0 && paddingFactor < 100, "RingBuffer size must be positive");
    ...
    public long take() {
        ...
        isTrue(nextCursor >= currentCursor, "Curosr can't move back");
        ...
        isTrue(flags[nextCursorIndex].get() == CAN_TAKE_FLAG, "Curosr not in can take status");
